...
public class PdfTableParseUtils {
    ...
    private static List<List<Line2D>> groupConnectedLines(List<Line2D> lineList) {
        int n = lineList.size();
        LineUnionFind uf = new LineUnionFind(n);
...
//                System.out.printf("rightLine.getX1()：" + rightLine.getX1());
     ...
     * 提取和处理文本块 过滤 排序
     */
    private static List<TextChunk> extractAndProcessTextChunks(
     ...
     * 判断文本块是否在单元格内
     */
    private static boolean isTextInCell(TextChunk textChunk,
                                        PdfCellPos cell) {
    ...
    private static PdfCellTextPos extractTextFromCell(
        ...
        // 遍历所有文本块，找出在单元格内的文本
        for (TextChunk textChunk : locationalResult) {
            try {
...
//         return text.trim();
    ...
    public static List<Map<String, String>> analyzeComplexTable(String filePath, int pageNumber) {
        ...
        try (CloseablePdfReader reader = new CloseablePdfReader(filePath);) {
            ...
            // 提取和处理文本块 过滤 排序
            List<TextChunk> locationalResult = extractAndProcessTextChunks(strategy);
